...
class BusScheduleViewModel(private val busScheduleRepository: BusScheduleRepository): ViewModel() {
    ...
    private val TIMEOUT_MILIS = 5_000L

    /**val listScreenUiState: StateFlow<ListScreenUiState> =
        busScheduleRepository.getAllSchedules().map { ListScreenUiState(it) }
            .stateIn(
    ...
    fun getScheduleFor(stopName: String): Flow<List<BusSchedule>> = busScheduleRepository.getScheduleFor(stopName)
...
/**
 * UI State for list of all bus schedules
 */
...
/**
 * UI State for a single stop name
 */
data class BusSheduleDetailsUiState(
